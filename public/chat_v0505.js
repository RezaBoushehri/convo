...
function getReplyInfo(quoteId) {
    ...
    const handle = sender === currentUser._id ? 'من' : sender;
...
function getForwardInfo(messageId) {
    // messageId is the short numeric id (e.g. "1000005"), same convention used by replyMessage()
    if (!messageId) return null;

    const messageElement = document.getElementById(`Message-${messageId}`);
    if (!messageElement) {
        console.warn("Message to forward not found in DOM:", messageId);
        return null;
    }

    const rawSender = messageElement.getAttribute('sender') || '';
    const displayName = messageElement.querySelector('h6') ? messageElement.querySelector('h6').innerText : (rawSender === currentUser._id ? 'من' : rawSender);

    const messageContent = messageElement.querySelector('.dataMessage');
    const messageText = messageContent ? messageContent.innerHTML.replace(/<br>/g, '\n').trim() : '';

    const truncatedMessage = messageText.length > 500
        ? messageText.substring(0, 500) + '...'
        : messageText;

    // Does the original message carry any files? If so, note it (files themselves are not re-forwarded).
    const hasFile = messageElement.querySelectorAll('[id^="file_"]').length > 0;

    return {
        originalId: (messageElement.dataset.mess_org_id) || `${roomID}-${messageId}`,
        sender: rawSender,
        handle: rawSender === currentUser._id ? 'من' : displayName,
        message: truncatedMessage,
        hasFile
    };
}
    ...
    function createUploadUI(username) {
        ...
        let name = 'N/A'
        if(user.username == currentUser._id){
            name = 'من'
        }else{
        ...
        Array.from(files).forEach((file,index)=>{
           ...
           $('#file-input_res').append(`
                   ...
                   <div class="col-auto rounded-1 badge px-3 gap-1 d-flex bg-primary shadow">
                       ...
                       <img nonce="<%= nonce %>" class="rounded-1" src="${fileURL}" width="auto" height="50" onerror="$(this).remove();$('#${index}_ICON').removeClass('d-none')" loading="lazy"  >
...
async function voice_upload(text,quote,blob){
    ...
    const xhr = new XMLHttpRequest();
            ...
            const formData = new FormData();
            formData.append("files", blob,`${currentUser.room}_${currentUser._id}.webm`);
...
async function sendMessage(text=null, fileData, quote, forward=null) {
    ...
    try {
        ...
        if (socket.connected) {
            ...
            console.log(data)
            if ((!data.message && !data.file && !data.forward) || !data.username) {
...
let typeUsername = currentUser._id;
    ...
    const decryptedMessage = await {
            // رمزگشایی مقادیر مختلف پیام با استفاده از شرط‌ها برای چک کردن وجود مقادیر
                ...
                forward: data.forward ? {
                    ...data.forward,
                    message: data.forward.message ? (data.forward.message) : data.forward.message,
                    sender: data.forward.sender ? (data.forward.sender) : data.forward.sender,
                } : null,
            };

    // Update the chat list (#side_contact) for whichever room this message belongs to —
    // this runs for messages we send (echoed back) as well as ones we receive.
    updateRoomListPreview(data.roomID, decryptedMessage, data.timestamp)

    const isForCurrentRoom = data.roomID === roomID;
    const isNearBottom =output.scrollHeight - output.scrollTop - output.clientHeight < 120
    if (ack.success && isForCurrentRoom) {
        ...
        if(lastMessage){
            ...
            // همیشه پیام را اضافه کن  
                await addMessageToChatUI(decryptedMessage,null,null,null,decryptedMessage.sender == currentUser._id ? '':"animate__faster animate__fadeInUp")
                    init_message_ui()
            if(shouldScroll){    
                    scrollDown()  
            ...
            //     else{
            //     if(decryptedMessage.sender != currentUser._id){
            //         unreadedScroll += 1      
...
//=================================================================
// Keep the chat list (#side_contact) in sync whenever a message is sent, received,
// edited, or deleted: updates the last-message preview text, the last-update
// timestamp, and re-sorts so the most recently active room floats to the top.

// Low-level setter used by new messages, edits, and deletes alike.
function setRoomListPreview(targetRoomID, previewText, timestamp, messageId) {
    if (!targetRoomID) return;

    const $li = $(`#side_contact li[id="${targetRoomID}"]`);
    if ($li.length === 0) return; // room not in the loaded sidebar (e.g. a brand-new room)

    $li.find('.message, .last-message, .col-auto.ms-0.text-muted').first().text(previewText);
    $li.attr('data-last-update', timestamp || new Date());
    $li.find('.jdate').text(formatDate(timestamp || new Date()));
    if (messageId) $li.attr('data-last-message-id', messageId);

    if (typeof sortRooms === 'function') sortRooms();
}

function updateRoomListPreview(targetRoomID, msg, timestamp) {
    if (!targetRoomID) return;

    const isOwn = msg.sender === currentUser._id;
    const prefix = isOwn ? `شما: ` : '';
    let previewText;
    if (msg.message) {
        previewText = msg.message.replace(/<[^>]*>/g, '').replace(/\n/g, ' ').trim();
    } else if (msg.forward) {
        previewText = 'پیام بازارسال شده';
    } else if (msg.file) {
        previewText = 'فایل';
    } else {
        previewText = '';
    ...
    setRoomListPreview(targetRoomID, prefix + previewText, timestamp, msg.id);

    // Optimistic unread bump for rooms other than the one currently open.
    // The server's own count_new_msg_room -> "roomList_newMessages" push follows
    // shortly after and carries the authoritative count, correcting any drift.
    if (targetRoomID !== roomID && !isOwn) {
        const $counter = $(`#side_contact li[id="${targetRoomID}"] .counter_message`);
        const current = parseInt($counter.text(), 10) || 0;
        $counter.text(current + 1).removeClass('d-none');
    }
}

// Returns the room id embedded in a full message id like "roomID-1000005"
function roomIDFromMessageId(messageId) {
...
socket.on("joined", (data) => {
    ...
    if(document.querySelector(".close")) document.querySelector(".close").click();
                ...
                <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-lg" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">        
                            ...
                            ${(data.room.admin == currentUser._id)?`
        ...
        Promise.resolve(
            ...
            $('#addMember_room_modal btn[data-submit="true"]').on('click',function(e){
                let res = false;
                console.log('ok')
        ...
        if($modal_body.find('.kick_myself').length == 0){
            $modal_body.append(`
                <span class="kick_myself my-1 gap-2 col-auto rounded-3 cursor-pointer btn btn-outline-danger" onclick="member_manage('kick','${currentUser._id}')"><i class="bi bi-person-fill-x"></i> ترک اتاق</span>
                            `)
        }
    })
    ...
    Promise
        .resolve(
            $('#side_contact li.bg-primary').removeClass('bg-primary').addClass('border-0'))
        .then(
            $(`#side_contact li[id="${data.room.roomID}"]`).addClass('bg-primary').removeClass('border-0'))
            ...
            if(lastMessageElm){
                ...
                if(inLast){
                    if(!inLast.querySelector('h6')){
                        ...
                        inLast.insertAdjacentHTML("afterbegin",`<h6 class="message-title" style="color:${messagesCreatedHandler[messagesCreatedHandler.length - 1] === currentUser._id ? 'var(--bs-blue)' : userColor}; font-style:italic;text-align:start;">${messagesCreatedHandler[messagesCreatedHandler.length - 1] === currentUser._id ?'من':messagesCreatedHandler[messagesCreatedHandler.length-1]}</h6>`)
                        // console.log('before border :',inLast.style.borderRad)
                        // inLast.style.borderRadius = '2px' ;
                        inLast.style.borderRadius = messagesCreatedHandler[messagesCreatedHandler.length - 1] === currentUser._id ? 'var(--user-border-radius) var(--user-border-radius) 5px var(--user-border-radius)' : ' var(--user-border-radius) var(--user-border-radius) var(--user-border-radius) 5px' ;
...
//     const ownMessage = data.sender === currentUser._id
...
//                     ${data.reply.sender == currentUser._id ? `من` : data.reply.handle ?? ''}
//                 </span>
...
function renderReply(reply){
    if(!reply) return ""
    const sender = reply.sender === currentUser._id? "من": reply.sender || ""
...
//     if(username === currentUser._id) return "من"
...
function addMessageToChatUI(data, prepend = false , isFirstMessage=false, isLastMessage=true ,animate = "", MessagePack = output.querySelectorAll('.MessagePack')) {
    ...
    const user = member_users.filter(user=> user._id === data.sender)[0]
    console.log(member_users)
    const user_name = user? `${user?.first_name} ${user?.last_name}`:data.sender
    const forwardUser = data.forward ? member_users.filter(u=> u._id === data.forward.sender)[0] : null
    const forwardSenderName = data.forward
        ? (data.forward.sender === currentUser._id
            ? `من`
            : (data.forward.senderName
                || (forwardUser ? `${forwardUser?.first_name} ${forwardUser?.last_name}` : null)
                || data.forward.senderUsername
                || data.forward.handle
                || 'کاربر'))
        : '';
    let contentToAdd = "";
    ...
    const ownMessage = data.sender === currentUser._id;
    console.log(ownMessage,data.sender)
    ...
    ? data.readUsers
        .map((r) => {
          ...
          return r.reaction
            ? `<span class='animate__animated animate__zoomIn   ${r.username == currentUser._id ? `ownReaction `:``} reactionMemEmoji ' ${r.username == currentUser._id ? `onclick="addStickerReaction('',${messageId})"`:''} user-id="${r.username}">${r.reaction}</span>`
            : "";
        })
        .join("")
    : "";
    ...
    ? data.readUsers
        .filter(r => r.username !== currentUser._id || (r.username === currentUser._id&& r?.reaction))
          .map((r) => {
            if(r.username === currentUser._id&& r.reaction !== ""){
            ...
            if(lastMessageElm){
                ...
                if(inLast){
                    if(!inLast.querySelector('h6')){
                        // console.log("last : ",inLast)
                        let userColor = `var(--color-peer-${lastMessageElm.getAttribute('sender')}) !important`
                        inLast.insertAdjacentHTML("afterbegin",`<h6 class="message-title text-primary" style="color:${messagesCreatedHandler[messagesCreatedHandler.length - 2] === currentUser._id ? '' : userColor}; font-style:italic;text-align:start;">${messagesCreatedHandler[messagesCreatedHandler.length - 2] === currentUser._id ?'من':messagesCreatedHandler[messagesCreatedHandler.length-2]}</h6>`)
                        // console.log('before border :',inLast.style.borderRad)
                        inLast.style.borderRadius = messagesCreatedHandler[messagesCreatedHandler.length - 2] === currentUser._id ? 'var(--user-border-radius) var(--user-border-radius) 5px var(--user-border-radius)' : ' var(--user-border-radius) var(--user-border-radius) var(--user-border-radius) 5px ' ;
                        // console.log('after border :',inLast.style.borderRad)
                    }
                }
                // messagesCreatedHandler=[]
   ...
   const telegramAudio = (src,id) => {
    ...
    <div id="Message-${messageId}" data-mess_org_id="${data.id}"
        ...
        <div class="message ${ownMessage? 'own':''} p-1 backdrop-blur-chat-bg col-auto ${data.sender== 'Heimdall'?"rounded m-auto":""}"
            ...
            ${handler()}
            ${data.forward && data.forward!==null ? `
            <div class="col-12 forwardedFrom small fst-italic mb-1 d-flex align-items-center gap-1 opacity-75" dir="auto">
                <i class="bi bi-arrow-90deg-right"></i>
                <span>بازارسال شده از</span>
                <img src="/portal/profile/img/${data.forward.senderUsername}" alt="Avatar" class="profile-avatar " style="width: 30px !important; height: 30px !important; border-radius: 50%; object-fit: cover;" >
                <span class="fw-semibold">${forwardSenderName}</span>
            </div>` : ''}
            ${data.reply && data.reply!==null ? `
            <div class="col-12 ">
                <div class="d-flex replyMessage gap-2 p-2 EmbeddedMessage cursor-pointer h-10rem position-relative m-2  peer-color-${ownMessage?`0`:`1`}" 
                    ...
                    <span class="col-auto message-title" 
                        dir="rtl" 
                        style="${ownMessage? `color: rgb(var(--user-fg-color));`:`color: var(--color-peer-${data.reply.sender});`} font-style:italic;text-align:end;">
                        ${data.reply.sender == currentUser._id ? `من` : 
                            ``
                        }
                    </span>
            ...
            <div class="m-0 row d-flex gap-2">
                <div class="text-content col-12" dir="auto">${data.forward.message
                    ...
                    : (data.forward.file ? `${capitalizeWord(data.forward.file.split('/')[0] ?? '')} File` : '')
                }</div>
            </div>` : ''}
            <div class="m-0 row d-flex gap-2">
                ${data.file && data.file!==null ? data.file.map(file => `
                    <div id="file_${file._id}" class="position-relative col-auto p-0">
                        ...
                        <div class="dropdown d-inline  position-absolute top-0 end-0 m-1 me-2" id="file_menu_${file._id}"
                                ...
                                data-file="${file.file}">
                            <button class="btn btn-sm  mt-1 p-0 rounded-3 shadow btn-outline-white backdrop-blur-chat-fg" data-bs-toggle="dropdown" aria-expanded="false" onclick="menu_file('${file._id}',${ownMessage || currentUser._id == '09173121943'})">
...
socket.on("delete_file",async(id)=>{
     if (id) {
        const fileEl = $(`#file_${id}`)
        if (fileEl) {
            Promise.resolve($(`#side_contact li[id="${currentUser.room}"]`).attr('data-last-update', new Date())).then(()=>{
...
function addStickerReaction(reaction,messageId) {
    ...
    socket.emit("addReaction", { username: currentUser._id, messageId: message, reaction:reaction });
    var stickerPicker = document.getElementById(`emoji-${messageId}`);
...
socket.on("reactionAdded", ({ messageId, username ,time  , reaction }) => {
    ...
    if (memberReaction) {
        ...
        if(userRect) {
            ...
            // userRect.innerHTML = `<span class='${username == currentUser._id ? `ownReaction `:``} reactionMemEmoji mx-1' user-id="${username}">${reaction}</span>`;

            // Create a new reaction element
          memberReaction.innerHTML += `<span class='animate__animated animate__zoomIn   ${username == currentUser._id ? `ownReaction `:``} reactionMemEmoji ' ${username == currentUser._id ? `onclick="addStickerReaction('',${spiltedId})"`:''} user-id="${username}">${reaction}</span>`
...
// _______________forward____________________
function forwardMessage(messageId) {
    const forwardInfo = getForwardInfo(messageId);
    if (!forwardInfo) {
        showAlert("پیام برای بازارسال یافت نشد", "warning");
        return;
    }
    openForwardPicker(forwardInfo);
}

function ensureForwardPickerModal() {
    let modal = document.getElementById('forwardPickerModal');
    if (modal) return modal;

    document.body.insertAdjacentHTML('beforeend', `
        <div id="forwardPickerModal" class="d-none" style="position:fixed;inset:0;z-index:2000;background:rgba(0,0,0,.5);display:flex;align-items:center;justify-content:center;">
            <div class="rounded-3 backdrop-blur-chat-bg p-3" style="width:min(420px,92vw);max-height:80vh;display:flex;flex-direction:column;gap:.5rem;">
                <div class="d-flex align-items-center justify-content-between">
                    <strong dir="auto">بازارسال به...</strong>
                    <button type="button" class="btn btn-sm btn-close" id="forwardPickerClose"></button>
                </div>
                <input type="text" id="forwardPickerSearch" class="form-control form-control-sm" placeholder="جستجوی کاربر..." dir="auto">
                <div id="forwardPickerList" class="list-group overflow-auto" style="max-height:50vh;"></div>
            </div>
        </div>
    `);
    modal = document.getElementById('forwardPickerModal');

    document.getElementById('forwardPickerClose').addEventListener('click', closeForwardPicker);
    modal.addEventListener('click', (e) => {
        if (e.target === modal) closeForwardPicker();
    });

    let searchDebounce;
    document.getElementById('forwardPickerSearch').addEventListener('input', (e) => {
        clearTimeout(searchDebounce);
        const term = e.target.value.trim();
        searchDebounce = setTimeout(() => {
            searchForwardTargets(term);
        }, 300);
    });
...
// Currently open forward payload, so a delayed search response can still act on the right message
let activeForwardInfo = null;

function openForwardPicker(forwardInfo) {
    activeForwardInfo = forwardInfo;
    const modal = ensureForwardPickerModal();
...
function searchForwardTargets(searchTerm) {
    const list = document.getElementById('forwardPickerList');
    list.innerHTML = `<div class="text-muted small p-2 text-center" dir="auto">در حال جستجو...</div>`;

    socket.emit("searchPrivateChatUsers", { searchTerm: searchTerm || '' }, (response) => {
        // If the modal was closed or a newer search started, ignore stale results
        if (!document.getElementById('forwardPickerModal')?.classList.contains('d-flex')) return;

        if (!response || !response.success) {
            list.innerHTML = `<div class="text-danger small p-2" dir="auto">${(response && response.error) || 'خطا در جستجو'}</div>`;
            return;
        }

        const users = response.users || [];
        if (users.length === 0) {
            list.innerHTML = `<div class="text-muted small p-2" dir="auto">کاربری یافت نشد</div>`;
            return;
        }

        list.innerHTML = users.map(user => `
            <button type="button" class="list-group-item list-group-item-action d-flex align-items-center gap-2" data-user-id="${user._id}" dir="auto">
                <img src="/portal/profile/img/${user.username}" alt="" style="width:32px;height:32px;border-radius:50%;object-fit:cover;" onerror="this.style.display='none'">
                <span class="text-truncate">${escapeHtml(user.fullName || user.username)}</span>
            </button>
        `).join('');

        list.querySelectorAll('[data-user-id]').forEach(item => {
            item.addEventListener('click', () => {
                if (!activeForwardInfo) return;
                forwardToUser(item.getAttribute('data-user-id'), activeForwardInfo);
                closeForwardPicker();
            });
        });
    });
...
function forwardToUser(targetUserId, forwardInfo) {
    socket.emit("startPrivateChat", { targetUserId }, (response) => {
        if (response && response.success && response.roomID) {
            sendForwardedMessage(response.roomID, forwardInfo);
        } else {
            showAlert((response && response.error) || "امکان بازارسال به این کاربر وجود ندارد", "danger");
        }
    });
}

function sendForwardedMessage(targetRoomID, forwardInfo) {
    if (!targetRoomID || !forwardInfo || !forwardInfo.originalId) return;

    const messID = `${targetRoomID}-${Date.now() + Math.random()}`;
    const dataEncrypt = {
        id: messID,
        username: currentUser._id,
        roomID: targetRoomID,
        ...
        message: '',
        file: null,
        forward: forwardInfo.originalId, // fully-qualified id of the original message (may be from another room)
        date: new Date(),
    };
    const dataShow = {
        id: messID,
        ...
        file: '',
        reply: null,
        forward: forwardInfo, // used only for the local, optimistic preview
        date: new Date(),
    };

    // Optimistic UI: only if the target room is the one currently open.
    // The canonical message (with the server-resolved forward preview) arrives
    // via the "chat" broadcast listener and replaces this once the ack lands.
    if (targetRoomID === roomID) {
        addMessageToChatUI({ ...dataShow, isPending: true }, null, null, null, "animate__faster animate__fadeInUp");
    }

    socket.emit("chat", dataEncrypt, (ack) => {
        if (ack && ack.success) {
            if (targetRoomID === roomID) {
                updateMessageStatus(ack.messageId, 'sent');
            } else {
                showAlert("پیام بازارسال شد", "success");
            }
        } else {
            if (targetRoomID === roomID) {
                updateMessageStatus(ack && ack.messageId, 'failed');
            }
            showAlert((ack && ack.message) || "بازارسال پیام ناموفق بود", "danger");
        }
    });
}
...
async function deleteMessage(messageId) {
    ...
    try {
        ...
        socket.emit('delete', { username :currentUser._id , messageId }, async (response) => {
            if (response && response.success) {
                // Successfully deleted on server
...
socket.on("delete", (messageId) => {
    ...
    const targetRoomID = roomIDFromMessageId(messageId);
    const $messageElement = $(`[data-mess_org_id="${messageId}"]`);
    const wasLastMessageInList = $(`#side_contact li[id="${targetRoomID}"]`).attr('data-last-message-id') === messageId;
    ...
    if ($messageElement.length > 0) {
        ...
        setTimeout(() => {
            ...
            if (wasLastMessageInList) {
                if (targetRoomID === roomID) {
                    // We're viewing this room — recompute the preview from whatever is now the last message in the DOM
                    const remaining = output.querySelectorAll('.messageElm')
                    const lastElm = remaining[remaining.length - 1]
                    const previewText = lastElm ? (lastElm.querySelector('.dataMessage')?.textContent.trim() || 'فایل') : ''
                    setRoomListPreview(targetRoomID, previewText, new Date(), null)
                } else {
                    setRoomListPreview(targetRoomID, 'پیامی پاک شده است', new Date(), null)
                }
            }
        }, 1000); // Remove from DOM
    } else if (wasLastMessageInList) {
        // Message wasn't rendered here (different room) but it was this room's last message
        setRoomListPreview(targetRoomID, 'پیامی پاک شده است', new Date(), null)
    }
});
socket.on("edit", (data) => {
    ...
    // If this was the room's last message, keep the sidebar preview in sync too
    if ($(`#side_contact li[id="${targetRoomID}"]`).attr('data-last-message-id') === messageId) {
        const previewText = (new_message || '').replace(/<[^>]*>/g, '').replace(/\n/g, ' ').trim();
        setRoomListPreview(targetRoomID, previewText, new Date(), messageId)
    }
});
// message menu
function messageMenu() {
    ...
    function openMenu(event, menu, element) {
        ...
        <button dir="auto" id="forwardMessage-${messageId}" class="d-flex list-group-item justify-content-between  btn visible col-12" >
            بازارسال
            <i class="bi bi-arrow-90deg-right"></i>
        </button>
        ...
        $(`#messageMenu #forwardMessage-${messageId}`).off().on("click",()=>{
...
//             var encryptedData =encryptMessage(currentUser._id)
